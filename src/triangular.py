...
# Initial velocity distribution: Triangular distribution
v_initial = np.random.triangular(left=-2.5, mode=0, right=2.5, size=n_particles)
# Initializes particle velocities with a triangular distribution ranging from -2.5 to 2.5, with the peak at 0.
...
plt.xlabel('Velocity')
plt.ylabel('Density')
plt.title('Initial Velocity Distribution (Triangular)')
plt.savefig('triangular_initial.png')
plt.show()
# Plots the initial velocity distribution and saves the figure as 'triangular_initial.png'.
...
for step in range(1, time_steps + 1):  # Fix the range to include the last time step
    for cell in cells:
...
plt.savefig('triangular_final_dsmc.png')
plt.show()
# Plots the final velocity distribution after all time steps and saves the figure as 'triangular_final_dsmc.png'.
